# ==============================================================================
# Script: Mixed-Effects Model & Directional Lambda Differences (Stage 2 vs Stage 1)
# Description: Tests directional hypotheses regarding the relative weights (lambdas) 
# of behavioral mechanisms shifting when the out-group becomes known, using 
# standardized attitudinal variables and a unified participant random effect.
# ==============================================================================
...
dfs_long <- dfs_long %>%
  mutate(
    # Standardize trust and conflict variables using scale()
...
# Model interacting core behavioral mechanisms with Stage (treatment)
lambda_mod <- compliance_extraction_OA ~
  # Main mechanisms interacted with Stage (T1 vs T2)
  ...
  # Hierarchical structure: Unified participant across both stages + group cluster
  (1 | participant.code) + (1 | gid.treat)

fit_mixed <- lmer(lambda_mod, data = dfs_long, control = ctrl, REML = FALSE)
summary(fit_mixed)
...
cat("\n=================================================================\n")
cat(" DIRECTIONAL HYPOTHESIS TESTS (Stage 2 - Stage 1) \n")
cat("=================================================================\n\n")
print(hypothesis_table, row.names = FALSE)

# --- 7. EXPORT TO WORD ---
out_file <- paste0(path_github, "Outputs/Lambda_Directional_Tests_UnifiedRE.docx")

datasummary_df(
  hypothesis_table,
  title = "One-Sided Tests for Shifts in Behavioral Mechanism Weights (Stage 2 \u2212 Stage 1)",
