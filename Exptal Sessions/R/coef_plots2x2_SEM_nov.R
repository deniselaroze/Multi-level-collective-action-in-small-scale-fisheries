...
p_compliance_T1 <- ggplot(compliance_results_SA, aes(x = est, y = label, group = Predictor)) +
  ...
  scale_size_manual(values = c("p < 0.05" = 6.0, "p >= 0.05" = 4.0), guide = "none") +
  # Add linewidth scale for error bars (replaces old scale_size_manual)
  scale_linewidth_manual(values = c("p < 0.05" = 2.0, "p >= 0.05" = 1.0), guide = "none") +
...
outcome_order_T1 <- c("Out-group (Unknown)", "In-group (S1)")
...
outcome_order_T2 <- c("Out-group (Known)", "In-group (S2)")
