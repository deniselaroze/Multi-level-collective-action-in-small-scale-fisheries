...
save_t2_sem_plot <- function(df, R, N, path_github) {
  ...
  output_file <- paste0(path_github, "Outputs/SEM_Turf_T2_plot_Rounds_ClusterLag_UpBfTURF_", R+8, "_to_", N+8, ".pdf")
  pdf(output_file, width = 12, height = 8)
...
save_dynamic_t2_sem_plot <- function(df, R, N, path_github) {
  ...
  # Save to PDF (rectangle shape)
  output_file <- paste0(path_github, "Outputs/SEM_Turf_T2_dynamic_plot_Rounds_ClusterLag_UpBfTURF_", R+8, "_to_", N+8, ".pdf")
  pdf(output_file, width = 12, height = 8)
  semPaths(
    fit,
    what = "std",
...
#T1
save_sem_plot(df, 1, 1, path_github)
save_dynamic_sem_plot(df, 8, 8, path_github)
#T2
save_t2_sem_plot(df, 1, 1, path_github)
save_dynamic_t2_sem_plot(df, 8, 8, path_github)
