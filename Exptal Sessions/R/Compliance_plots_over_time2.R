...
p_compliance_T1 <- ggplot(compliance_results_SA, aes(x = est, y = label, group = Predictor)) +
  ...
  theme_minimal() +
  theme(
    plot.title = element_text(hjust = 1, size = 15, face = "bold"), 
...
p_compliance_T2 <- ggplot(compliance_results_SA_T2, aes(x = est, y = label, group = Predictor)) +
  ...
  labs(
    ...
    x = "Beta Coefficient (95% CI)", # Central x-axis title
    y = ""
  ) +
  theme_minimal() +
  theme(
...
outcome_order <- c(
  "Out-group (Unknown)", "In-group (s1)",
  "Out-group (Known)", "In-group (s2)"
)
...
p_beliefs <- ggplot(combined_beliefs, aes(x = est, y = Outcome, color = Predictor, shape = Predictor)) +
  ...
  geom_text(data = subset(combined_beliefs, Significance == "p < 0.05"),
            aes(label = round(est, 2)),
            position = position_dodge(width = 0.5),
            vjust = -1,
            size = 5,
  ...
  scale_size_manual(values = c("p < 0.05" = 1, "p >= 0.05" = 0.5), guide = "none") +
  labs(
    title = "b) Beliefs about Behavior in Shared Area",
    x = NULL, # Remove individual x-axis titles
    y = "Experimental Stages: S1 (Unknow Out-group), S2 (Know Out-group)"
  ) +
  theme_minimal() +
  theme(
    plot.title = element_text(hjust = 1, size = 15, face = "bold"),
...
ggsave(
  paste0(path_github, "Outputs/Combined_SEM_Plot4.tiff"), 
  final_plot, 
  width = 18,
  height = 12
)
